...
function drawBackground() {
    ...
    drawHill(hill2BaseHight, hill2Amplitude, hill2Stretch, "#388ec3");

    tree.forEach((tree) => drawTree(tree.x, tree.color));
}

function drawHill(baseHight, amptitude, stretch, color) {
    ctx.beginPath();
    ctx.moveTo(0, window.innerHeight);
    ctx.lineTo(0, getHillY(0, baseHight, amptitude, stretch));
    for (let i = 0; i < window.innerWidth; i++) {
        ctx.lineTo(i, getHillY(i, baseHight, amptitude, stretch));
    }
...
function drawTree(x, color) {
    ...
    ctx.beginPath();
    ctx.moveTo(-treeCrownWidth / 2, -treeCrownHeight);
    ctx.lineTo(0, -(treeCrownHeight + treeCrownHeight));
    ctx.lineTo(treeCrownWidth / 2, -treeCrownHeight);
    ctx.fillStyle = color;
    ctx.fill();
    ctx.restore();
}

function getHillY(windowX, baseHight, amptitude, stretch) {
...
function getTreeY(x, baseHight, amptitude) {
    const sineBaseY = window.innerHeight - baseHight;
    return Math.sinus(x) + amptitude + sineBaseY;
}
